...
class _CleanupSettingsState extends ConsumerState<CleanupSettings> {
  ...
  int get _calculatedStep {
    ...
    if (state.assetCount != null && state.assetCount! > 0) return 2;
    if (state.selectedDate != null) return 1;
    return 0;
  }
  ...
  Future<void> _scanAssets() async {
    ...
    final state = ref.read(cleanupProvider);
    if (state.assetCount != null && state.assetCount! > 0) {
  ...
  Future<void> _deleteAssets() async {
    ...
    if (state.assetsToDelete.isEmpty || state.selectedDate == null) return;
  ...
  Widget build(BuildContext context) {
    ...
    final hasScanned = state.assetCount != null;
    final hasAssets = hasScanned && state.assetCount! > 0;
      ...
      child: Stepper(
        ...
        steps: [
          ...
          Step(
            ...
            title: Text(
              'cleanup_cutoff_date_title'.t(context: context),
            ...
            content: Column(
              ...
              children: [
                Text('cleanup_description'.t(context: context), style: context.textTheme.bodyMedium),
                const SizedBox(height: 16),
                ElevatedButton.icon(
                  onPressed: _selectDate,
                  icon: const Icon(Icons.calendar_today),
                  label: Text(
                    hasDate ? 'change'.t(context: context) : 'cleanup_select_cutoff_date'.t(context: context),
                  ),
          ...
          Step(
            ...
            title: Text(
              'cleanup_step2_title'.t(context: context),
              style: context.textTheme.titleMedium?.copyWith(
                fontWeight: FontWeight.w600,
            ...
            subtitle: hasScanned
                ? Text(
                    'cleanup_found_assets'.t(context: context, args: {'count': state.assetCount.toString()}),
                    style: context.textTheme.bodyMedium?.copyWith(
                      color: state.assetCount! > 0
                          ? context.colorScheme.primary
            ...
            content: Column(
              crossAxisAlignment: CrossAxisAlignment.stretch,
              children: [
                Text('cleanup_step2_description'.t(context: context), style: context.textTheme.bodyMedium),
                const SizedBox(height: 16),
                ElevatedButton.icon(
                  ...
                  label: Text(
                    state.isScanning
                        ? 'cleanup_scanning'.t(context: context)
                        : hasScanned
                        ? 'rescan'.t(context: context)
                        : 'cleanup_scan_assets'.t(context: context),
                  ),
                  style: ElevatedButton.styleFrom(minimumSize: const Size(double.infinity, 48)),
                ),
                if (hasScanned && state.assetCount == 0) ...[
                  const SizedBox(height: 16),
                  Container(
          ...
          Step(
            ...
            title: Text(
              'cleanup_step3_title'.t(context: context),
              style: context.textTheme.titleMedium?.copyWith(
                fontWeight: FontWeight.w600,
            ...
            content: Column(
              ...
              children: [
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              'cleanup_step3_warning'.t(context: context),
                      ...
                      if (hasAssets)
                        Text(
                          'cleanup_step3_summary'.t(
                            context: context,
                            args: {'count': state.assetCount.toString(), 'date': _formatDate(state.selectedDate!)},
                          ),
                          style: context.textTheme.bodyMedium,
                        ),
                    ],
                ...
                Text(
                  'cleanup_assets_verified'.t(context: context),
                ...
                ElevatedButton.icon(
                  ...
                  label: Text(
                    state.isDeleting
                        ...
                        : 'cleanup_delete_from_device'.t(context: context),
                  ),
...
class _DeleteConfirmationDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: Text('cleanup_confirm_title'.t(context: context)),
      content: Text(
      ...
      actions: [
        ...
        TextButton(
          ...
          child: Text(
            'cleanup_delete'.t(context: context),
